...
int main(){
	...
	for (i=0;i<80;i++){
		s=(p[i].key+strlen(p[i].name)^3)%mod;
		if (Hash[s]==-1) Hash[s]=i;
		else{
			if (buf<90) Hash[buf++]=i;
			else{
				for (j=0;j<80;j++)
					if (Hash[j]==-1) {Hash[j]=i; break;}
			}
		}
	}
	for (i=0;i<90;i++)
	...
	for (i=0;i<80;i++){
		...
		for (j=0;j<strlen(str);j++)
			if (str[j]>='A' && str[j]<='Z')
				...
				code=(code*10)%MOD;
		code=(code+strlen(str)^3)%mod; tmp++;
		if (Hash[code]!=-1){
			if (!strcmp(p[Hash[code]].name,str)) printf("%dst name found in Hash[%d] after searching %d times.\n",i,code,tmp);
			else{
				j=80; tmp++;
				while (j!=90 && strcmp(p[Hash[j]].name,str)!=0) {j++; tmp++;}
				if (j==90){
					j=0; 
					while (strcmp(p[Hash[j]].name,str)) {j++; tmp++;}
				}
	...
	code=(code+strlen(inp)^3)%mod; tmp++;
	if (Hash[code]!=-1){
		if (!strcmp(p[Hash[code]].name,inp)){
			...
			j=80; tmp++;
			while (j!=90 && strcmp(p[Hash[j]].name,inp)!=0) {j++; tmp++;}
			if (j==90){
				j=0; 
				while (strcmp(p[Hash[j]].name,inp) && j<80) {j++; tmp++;}
				if (j==80) {printf("Not found!\n"); return 0;}
			}
			printf("The input name found in Hash[%d] after searching %d times.\n",j,tmp);
		}
